...
# --- RULES DATA (Moved from rules.py to be self-contained) ---
            ...
            with col2:
                if facility['type'] == 'Special' and facility['status'] != 'Idle':
                    if st.button("Cancel Order", key=f"cancel_{facility['id']}"):
                        cancelled_order = facility['status']
                        facility['status'] = 'Idle'
                        facility['order_progress'] = 0
                        facility['order_duration'] = 0
                        add_log_entry(st.session_state.data['campaign']['current_day'], f"{char_name} cancelled the order '{cancelled_order}' at the {facility['name']}.")
                        st.warning(f"Order '{cancelled_order}' has been cancelled.")
                        time.sleep(1)
                        st.rerun()
